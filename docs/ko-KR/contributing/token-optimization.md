...
- 관련 없는 작업은 새 세션에서 시작
- 컨텍스트 집약적 작업 전에 `/egc-checkpoint`으로 상태 저장
- `/egc-sessions`으��� 과거 세션을 확인하고 재개
...
| 반복적인 빌드 수정 | 언어별 리졸버 사용 (일반 대신 `/egc-go-build`) |
| 문서 업데이트 | 수동 프롬프트 대신 `/egc-update-docs` 사용 |
| 아키텍처 결정 | 명확한 범위 경계와 함께 `@architect` 사용 |

## 토큰 효율을 위한 지속적 학습

`/egc-learn` 커맨드는 세션에서 패턴을 추출하여 재사용 가능한 스킬로 저장합니다. 향후 세션에서 컨텍스트를 다시 설명할 필요가 줄어듭니다:

```bash
/egc-learn          # 현재 세션에서 패턴 추출
/egc-evolve         # 인스팅트를 효율적인 스킬로 클러스터링
```
