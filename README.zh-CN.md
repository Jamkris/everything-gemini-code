...
**来自 Anthropic 黑客马拉松获胜者的完整 Gemini CLI 配置集合。**
...
> ⚠️ **重要提示：** Gemini CLI 插件无法自动分发 `rules`，需要手动安装：
...
# 复制规则（应用于所有项目）
cp -r everything-gemini-code/rules/* ~/.gemini/rules/
```
...
|
|-- rules/            # 始终遵循的指南（复制到 ~/.gemini/rules/）
|   |-- security.md         # 强制性安全检查
...
# 复制规则
cp everything-gemini-code/rules/*.md ~/.gemini/rules/
...
将 `hooks/hooks.json` 中的钩子复制到你的 `~/.gemini/settings.json`。

#### 配置 MCP

将所需的 MCP 服务器从 `mcp-configs/mcp-servers.json` 复制到你的 `~/.gemini.json`。
...
自实验性推出以来，我一直在使用 Gemini CLI。2025 年 9 月，与 [@DRodriguezFX](https://x.com/DRodriguezFX) 一起使用 Gemini CLI 构建 [zenith.chat](https://zenith.chat)，赢得了 Anthropic x Forum Ventures 黑客马拉松。
...
- **精简指南（从这里开始）：** [The Shorthand Guide to Everything Gemini CLI](https://x.com/affaanmustafa/status/2012378465664745795)
- **详细指南（高级）：** [The Longform Guide to Everything Gemini CLI](https://x.com/affaanmustafa/status/2014040193557471352)
