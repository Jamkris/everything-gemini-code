...
function getChecks() {
  ...
  // Note: For Gemini CLI, commands use toml format
  ...
  return [
    ...
    },
    {
      ...
      pass: typeof packageJson.scripts?.test === 'string' && packageJson.scripts.test.includes('tests/run-all.js'),
      fix: 'Update package.json test script to run tests/run-all.js.',
    },
    {
    ...
    },
    {
      ...
      points: 2,
      scopes: ['repo', 'commands'],
      path: 'commands/security-scan.toml',
      description: 'Security scan command exists',
      pass: fileExists('commands/security-scan.toml'),
      fix: 'Add commands/security-scan.toml with scan and remediation workflow.',
    },
    {
      id: 'cost-skill',
...
function printText(report) {
  console.log(`Gemini Harness Audit (${report.scope}): ${report.overall_score}/${report.max_score}`);
  console.log('');
  ...
  if (failed.length > 0) {
    console.log('');
    console.log('Top Actions to resolve:');
