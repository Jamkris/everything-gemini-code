...
function runTests() {
  ...
  if (test('getGeminiDir returns path under home', () => {
    const geminiDir = utils.getGeminiDir();
    const homeDir = utils.getHomeDir();
    assert.ok(geminiDir.startsWith(homeDir), 'Gemini dir should be under home');
    assert.ok(geminiDir.includes('.gemini'), 'Should contain .gemini');
  })) passed++; else failed++;

  if (test('getSessionsDir returns path under Gemini dir', () => {
    const sessionsDir = utils.getSessionsDir();
    const geminiDir = utils.getGeminiDir();
    assert.ok(sessionsDir.startsWith(geminiDir), 'Sessions should be under Gemini dir');
  ...
  if (test('getSessionIdShort falls back to project name', () => {
    ...
    try {
      ...
      if (original) process.env.GEMINI_SESSION_ID = original;
    }
  })) passed++; else failed++;

  if (test('getSessionIdShort returns last 8 characters', () => {
    const original = process.env.GEMINI_SESSION_ID;
    process.env.GEMINI_SESSION_ID = 'test-session-abc12345';
    try {
      assert.strictEqual(utils.getSessionIdShort(), 'abc12345');
    } finally {
      if (original) process.env.GEMINI_SESSION_ID = original;
      else delete process.env.GEMINI_SESSION_ID;
    }
  })) passed++; else failed++;

  if (test('getSessionIdShort handles short session IDs', () => {
    const original = process.env.GEMINI_SESSION_ID;
    process.env.GEMINI_SESSION_ID = 'short';
    try {
