...
This document captures **undocumented but enforced constraints** of the Gemini CLI plugin manifest validator.
...
If you edit `.gemini-plugin/plugin.json`, read this first.

---

## Summary (Read This First)

The Gemini plugin manifest validator is **strict and opinionated**.
...
* `gemini plugin validate` is stricter than some marketplace previews
...
Gemini CLI v2.1+ **automatically loads** `hooks/hooks.json` from any installed plugin by convention. If you also declare it in `plugin.json`, you get:
...
**Root cause:** Gemini CLI CLI changed behavior between versions:
...
This structure has been validated against the Gemini plugin validator.
...
```bash
gemini plugin validate .gemini-plugin/plugin.json
```
